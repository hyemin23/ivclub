...
interface SortableLookbookItemProps {
    ...
    idx: number;
    isExporting: boolean;
    store: any; // Using explicit type if possible, or any for mapped store
    onDelete: (id: string) => void;
}

export const SortableLookbookItem: React.FC<SortableLookbookItemProps> = ({ 
    img, 
    idx, 
    isExporting, 
    store,
    ...
    return (
        <div ref={setNodeRef} style={style}>
            <DownloadableSection fileName={`lookbook_${idx + 1}_${store.name}`}>
                <div className="relative group">
                    ...
                    {!isExporting && (
                        <div className="absolute top-4 left-4 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                            ...
                            {/* Existing Controls */}
                            <button 
                                onClick={() => store.moveLookbookImage(img.id, 'up')}
                                disabled={idx === 0}
                            ...
                            <button 
                                onClick={() => store.moveLookbookImage(img.id, 'down')}
                                disabled={idx === store.lookbookImages.length - 1}
                                className="p-2 bg-white/90 shadow-lg text-slate-700 hover:text-indigo-600 rounded-lg disabled:opacity-50 hover:scale-110 transition-all border border-slate-200"
                            >
