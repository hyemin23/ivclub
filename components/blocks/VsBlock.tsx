...
const VsBlock: React.FC<VsBlockProps> = ({ content }) => {
    const title = content?.title || "Why Choose Us?";
    const items = content?.items || [
        { label: "High Quality Fabric", good: true },
        { label: "Cheap Material", good: false },
        { label: "Perfect Fit", good: true },
        { label: "Loose Fit", good: false },
    ];
            ...
            <div className="flex gap-4">
                ...
                <div className="flex-1 bg-blue-50 border-2 border-blue-500 rounded-xl p-4 relative overflow-hidden">
                    ...
                    <div className="mt-6 space-y-3">
                         {items.filter(i => i.good).map((item, idx) => (
