...
/**
 * 2️⃣ Color Variation Service
 * Includes Vision Analysis (Color Extraction) + Text-to-Image Edit
 */

// Import Server Action for Sharp Processing
import { preprocessImageForAnalysis } from '@/app/actions/imageActions';
...
export const changeColorVariant = async (
  ...
  // Step 0: Smart Pre-processing (Nukki Shot Handling)
  // Calls Server Action (Node.js environment)
  let processedRef = referenceColorImage;
  try {
    processedRef = await preprocessImageForAnalysis(referenceColorImage);
  } catch (e) {
  ...
  const descriptionPrompt = `
      Analyze this image and describe the dominant color or fabric finish of the CLOTHING ITEM in detail.
      
      CRITICAL INSTRUCTION (SMART EXTRACTION):
      1. IGNORE the background completely (white, gray, or transparent).
      2. If the image has a white background, look ONLY at the object in the center.
      3. Focus on the pigment of the fabric.
      
      Example Output: "Deep indigo blue with a slight faded denim texture", "Matte olive green", "Metallic silver".
      Keep it concise (under 10 words).
  `;
  // ... (rest as before until end of function, removing local helper)


  console.log("🎨 Extracting Color Description (Smart Mode)...");
  // Use the processed (masked/trimmed) image
  const analysisResult = await generateContentSafe(descriptionPrompt, [fileToPart(processedRef)], {
    taskType: 'TEXT',
...
export const generateBackgroundVariations = async (
  ...
  for (let i = 0; i < variations.length; i += chunkSize) {
    ...
    onStatusUpdate?.(`시안 생성 중 (${i + 1}~${Math.min(i + chunkSize, variations.length)}/${variations.length})... ⚡️`);
